...
const KelolaMateri = ({ isGridUI = false }: { isGridUI?: boolean }) => {
  ...
  const handleDownload = async (fileUrl: string, id: string) => {
    try {
      ...
      if (contentDisposition && contentDisposition.includes('filename')) {
        ...
        else if (contentType.includes('wordprocessingml') || contentType.includes('msword')) fileName += '.docx';
        else if (contentType.includes('presentationml') || contentType.includes('ms-powerpoint')) fileName += '.pptx';
      ...
      console.error("Gagal mendownload materi", error);
    } finally {
      setTimeout(() => {
    ...
    (itemMateri: Record<string, unknown>, columnKey: Key) => {
      ...
      switch (columnKey) {
        case "fileMateri":
          ...
          const isDownloading = downloadingIds[itemMateri.id as string];
          return (
            <div 
              className="flex items-center gap-2 cursor-pointer hover:text-brand transition-colors"
              onClick={() => handleDownload(cellValue as string, itemMateri.id as string)}
            >
              <Image 
                alt="Doc Icon" 
                height={20} 
              ...
              <span className="text-sm text-blue-600 underline">
                {isDownloading ? "Mengunduh..." : "Download materi"}
              </span>
            </div>
          );
        case "aksi":
            ...
            <Dropdown>
              ...
              <DropdownMenu>
                <DropdownItem
                  ...
                  onPress={() => {
                    setSelectedId(String(itemMateri.id));
  ...
  const columns = role === "peserta" ? LISTS_PESERTA_MATERI : LISTS_KELOLA_MATERI;
  ...
  return (
    <section className={isGridUI ? "w-full max-w-6xl mx-auto px-4 z-10" : ""}>
      {isLoadingSession ? (
        ...
        <div className={isGridUI ? "bg-white p-6 rounded-xl shadow-lg border border-gray-100" : ""}>
          {isGridUI && (
            <div className="mb-6">
              <h1 className="text-3xl font-bold text-brand mb-2">Materi Training</h1>
              <p className="text-gray-600">Lihat dan unduh materi training yang sudah diikuti.</p>
            </div>
          )}
          
          <DataTable
            buttonTopContentLabel={role !== "peserta" ? "Tambah Materi" : undefined}
            columns={columns}
